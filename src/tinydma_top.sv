...
module tinydma_top #(
  ...
  // Register/controller handshake and externally visible channel status.
  logic [N_CH-1:0] start_clear;
  logic [N_CH-1:0] chan_active;
  logic [N_CH-1:0] chan_done;

  // Scheduler grants one non-active started channel at a time.
  logic                  sched_valid;
  logic [$clog2(N_CH)-1:0] sched_idx;
  logic                  sched_advance;

  // Decoded register configuration for each channel.
  ...
  // Internal byte-wide memory interface between DMA and PSRAM controller.
  ...
  // Collapse the flattened per-channel status into small vectors for cfg_reg
  // readback and the top-level status pins.
